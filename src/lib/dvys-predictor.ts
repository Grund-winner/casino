...
export function calcConfidence(
  ...
  if (chosenCat === trend.trend) conf += Math.round(trend.strength * 15);
  else if (isForced) conf -= 5;
...
export function predict(
  ...
  if (isForced) { signals.push('rotation_securite'); signalTypes.push('info'); }
  if (forceTransition) { signals.push('transition_bas_vers_mid'); signalTypes.push('up'); }
  ...
  return { prediction: pred, confidence: conf, signals, signalTypes, category: chosenCat, rounds: n, avg: Math.round(avg(fullHistory) * 100) / 100, std: Math.round(sd(fullHistory) * 100) / 100, accuracy };
}
...
export function extractCoefs(data: unknown): number[] {
  ...
  for (const item of data) {
    ...
    if (Array.isArray(obj.finalValues)) {
      for (const v of obj.finalValues) { const fv = parseFloat(String(v)); if (fv > 0) values.push(fv); }
      continue;
    }
    const coef = obj.coef ?? obj.crash ?? obj.value ?? obj.topCoefficient ?? obj.multiplier ?? obj.coefficient;
    if (coef !== undefined) { const fv = parseFloat(String(coef)); if (fv > 0) values.push(fv); }
  }
  return values;
}
