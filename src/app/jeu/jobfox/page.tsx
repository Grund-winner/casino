...
export default function JobFoxPage() {
  ...
  return (
    <div className="jf-page">
      {/* Particles */}
      <div className="jf-particles">
        {particles.map((p, i) => <div key={i} className="jf-particle" style={{ left: p.left+'%', top: p.top+'%', animationDelay: p.delay+'s', animationDuration: p.dur+'s' }} />)}
      </div>

      {/* Loading */}
      {loading && <div className={`jf-loading${!loading ? ' hide' : ''}`}>
        <div className="jf-loading-logo">THE FOX&apos;s JOB</div>
        <div className="jf-loading-bar"><div className="jf-loading-fill" style={{ width: loadPct+'%' }} /></div>
        <div className="jf-loading-tag">DVYS AI Prediction</div>
      </div>}

      {/* Result banner */}
      <div className={`jf-result-banner ${resultClass} ${showResult ? 'show' : ''}`}>
        <span className="jf-rb-main">{resultMain}</span>
        <span className="jf-rb-sub">{resultSub}</span>
      </div>

      {/* Top bar */}
      <div className="jf-top">
        <button className="jf-back" onClick={doBack}>
        ...
        <div className="jf-logo"><span className="jf-logo-t">THE FOX&apos;s <span>JOB</span></span></div>
      </div>

      {/* DVYS badge */}
      <div style={{ position:'absolute', left:'50%', top: 'max(env(safe-area-inset-top,10px),10px)', transform:'translateX(-50%)', display:'flex', flexDirection:'column', alignItems:'center', zIndex: 21 }}>
        <span className="jf-dvys-logo">DVYS</span>
        <span className="jf-dvys-sub">AI Prediction</span>
      </div>
          ...
          {gridData?.map((row, ri) => (
            <div key={ri} className={`jf-row${ri < revealedRows ? ' visible' : ''}`}>
              {row.map((cell, ci) => {
                ...
                const ch = mob ? 49 : 58;
                let cls = 'jf-cell ';
                if (!revealed) cls += 'jf-cell-hidden';
                else if (cell.safe) cls += 'jf-cell-safe';
                else cls += 'jf-cell-trap';
                if (isPath) cls += ' jf-cell-path';
                if (isStop) cls += ' jf-cell-stop';
                return (
                  <div key={ci} className={cls} style={{ width: cw+'px', height: ch+'px' }}>
                    {revealed && cell.safe && <><span className="jf-cell-icon">{'\u{1FA99}'}</span><span className="jf-cell-mult">\u00d7{cell.mult.toFixed(2)}</span></>}
                    {revealed && !cell.safe && <><span className="jf-cell-trap-icon">{'\u26D3\uFE0F'}</span><span className="jf-cell-trap-label">TRAP</span></>}
                  </div>
                );
              })}
            </div>
      ...
      {/* Popover */}
      <div className={`jf-popover-ov${showPopover ? ' show' : ''}`} onClick={() => setShowPopover(false)} />
      <div className={`jf-popover${showPopover ? ' show' : ''}`}>
        <div className="jf-pop-title">Choisir la difficult\u00e9</div>
        <div className="jf-pop-list">
          {(Object.keys(DIFFICULTIES) as DiffKey[]).map(k => {
            const dd = DIFFICULTIES[k];
            return (
              <div key={k} className={`jf-pop-opt${k === cur ? ' act' : ''}`} onClick={() => selDiff(k)}>
                <div className="jf-pop-di">{ICONS[k]}</div>
                <div>
                  <div className="jf-pop-dn">{dd.label}</div>
                  <div className="jf-pop-dr">{dd.ratio} \u00b7 {dd.pct} de chances</div>
                  <div className="jf-pop-dm">{dd.mult.length} marches \u00b7 Max {dd.mult[dd.mult.length-1].toFixed(2)}x</div>
                </div>
              </div>
            );
          })}
      ...
      {/* Fox run animation */}
      {showFox && <div className="jf-fox-run" style={{ left:'110%', transition:'left 7s cubic-bezier(.22,.61,.36,1)' }}>
        <div className="jf-fox-text">CASH OUT !</div>
        <div className="jf-fox-img-wrap"><img className="jf-fox-img" src="/fox_running.png" alt="fox" /></div>
      </div>}

      {/* Bottom controls */}
      <div className="jf-bot">
        <button className="jf-diff-btn" onClick={() => setShowPopover(true)}>
          <span className="jf-diff-icon">{'\u2699\uFE0F'}</span>
          <div className="jf-diff-text">
            <span className="jf-diff-name">{d.label}</span>
            <span className="jf-diff-sub">{d.ratio.split(' / ')[0]}/{d.ratio.split(' / ')[1]} \u00b7 {d.pct}</span>
          </div>
        </button>
        <button className={`jf-predict-btn${busy ? ' busy' : ''}`} onClick={predict}>
          {busy ? <>ANALYSE...<span className="jf-btn-sub">DVYS AI</span></> : <>PR\u00C9DIRE<span className="jf-btn-sub">DVYS AI</span></>}
        </button>
      </div>
    </div>
  );
